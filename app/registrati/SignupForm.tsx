...
import { submitSignup, type SignupRole, type SignupState } from "./actions";
...
const ROLE_ORDER: SignupRole[] = ["tutor", "agency", "student"];
...
export default function SignupForm({
  ...
  return (
    ...
    >
      ...
      <div>
        <label htmlFor="role" className="text-sm font-bold text-[var(--navy)]">
          {s.roleLabel}
        </label>
        <select
          id="role"
          ...
          className={`mt-2 ${FIELD} ${err.role ? FIELD_BAD : FIELD_OK}`}
        >
          {ROLE_ORDER.map((r) => (
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-2">
        <div>
          <label htmlFor="name" className="text-sm font-bold text-[var(--navy)]">
            {s.nameLabel}
          </label>
          <input
            id="name"
      ...
      <div className="mt-5">
        ...
        <textarea
          ...
          className={`mt-2 resize-y ${FIELD} ${FIELD_OK}`}
        />
      </div>
